...
module.exports =
{
    ...
    async execute(interaction)
    {
        ...
        if (!interaction.options.getString('command'))
        {
            ...
            response.push(`:wave: **Greetings!** My name is Blue-chan! I have **${commands.size} commands** (with sub-commands):`);
            response.push(``);
            response.push(commands.map(command => `**${command.data.toJSON().name}**`).join('\n'));
            response.push(``);
            response.push('Blue-chan created by **defnotreal_** and **Blue-kun**');
        }
        else
        {
